...
/*
	...
	@retval			无
 */
void OLED_ShowPicture2(uint8_t x0,uint8_t y0,uint8_t x1,uint8_t y1,uint8_t BMP[])
{
	uint32_t j=0;
	uint8_t x=0,y=0;
	if(y%8==0)y=0;
...
/*
	...
	@param			x0：起始列地址
					...
					BMP[]：存放图片代码的数组
	@retval			无
	@remarks 不用OLED_Refresh();就可以显示
 */
void OLED_ShowPicture(unsigned char x, unsigned char y,unsigned char width, unsigned char hight,unsigned char BMP[])
{
	OLED_ShowPicture2(x, y, x + width, y + hight / 8, BMP);
}
